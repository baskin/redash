  ...
  const executeQuery = useImmutableCallback((maxAge = 0, queryExecutor) => {
    ...
    newQueryResult
      ...
      .then(queryResult => {
        if (queryResultInExecution.current === newQueryResult) {
          ...
          if (executionState.loadedInitialResults) {
            notifications.showNotification("Insights (by Unicommerce)", `${query.name} updated.`);
          }
      ...
      .catch(queryResult => {
        if (queryResultInExecution.current === newQueryResult) {
          if (executionState.loadedInitialResults) {
            notifications.showNotification("Insights (by Unicommerce)", `${query.name} failed to run: ${queryResult.getError()}`);
          }
